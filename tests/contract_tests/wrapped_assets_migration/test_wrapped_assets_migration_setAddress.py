...
class MainnetOperationTestCaseWRAPPED_ASSETS_MIGRATION(TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        with open(join(dirname(__file__), f'', f'setAddress.json')) as f:
            operation = json.loads(f.read())

        cls.entrypoint = f'setAddress'
        cls.operation = operation
        # cls.maxDiff = None

    def test_parameters_wrapped_assets_migration(self):
    ...
    def test_lazy_storage_wrapped_assets_migration(self):
