...
class MainnetOperationTestCaseFXHASH_METADATA(TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        with open(join(dirname(__file__), f'', f'set_splits.json')) as f:
            operation = json.loads(f.read())

        cls.entrypoint = f'set_splits'
        cls.operation = operation
        # cls.maxDiff = None

    def test_parameters_fxhash_metadata(self):
    ...
    def test_lazy_storage_fxhash_metadata(self):
